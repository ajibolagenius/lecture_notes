...
.main-header {
  ...
  /* 2. Align items on the MAIN axis (horizontal)
     This puts the .logo on the left and .navbar on the right.
  */
  justify-content: space-between;

  /* 3. Align items on the CROSS axis (vertical)
     This centers them vertically with each other.
  */
  align-items: center;
...
/* --- Navigation Layout (Nested Flexbox) --- */
.navbar ul {
  /* 1. Turn the <ul> into a Flex Container */
  display: flex;
...
/* --- Badge Layout (Positioning) --- */

/* 1. This is the ANCHOR.
   We must set 'position: relative' on the <li>
   so the badge positions itself relative to this list item.
*/
.nav-sale {
  position: relative;
}
...
/* 2. This is the FLIER.
   We use 'position: absolute' to pull the badge
   out of the normal flow.
*/
.sale-badge {
  position: absolute;
  top: 0; /* Position at the top edge of the <li> */
  right: 0; /* Position at the right edge of the <li> */

  /* Nudge it a bit for looks */
