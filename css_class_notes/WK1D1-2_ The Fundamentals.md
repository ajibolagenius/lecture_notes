...
Create this exact structure. The `id` and `class` names are important.
...
<body>
  ...
  <div id="container">
    <header id="main-header">
      <h1 class="site-title">My Name Here</h1>
      <p class="tagline">Student, Developer, & Avid Coffee Drinker</p>
    </header>
    ...
    <main id="content">
      <section class="section-box">
        <h2>About Me</h2>
        <p class="intro">
          Write a short paragraph about yourself here. Lorem ipsum dolor sit amet,
          consectetur adipiscing elit. Quisque nisl eros, pulvinar facilisis
          justo mollis, auctor consequat urna.
        </p>
      </section>
...
#### Part 2: The CSS Styling (`style.css`)

Create this exact structure. The `id` and `class` names are important.

``` css
/* STEP 1: General & Inherited Styles
  We use the 'body' TYPE selector to set global defaults
  that many other elements will INHERIT.
*/
body {
...
/* STEP 2: Layout Selectors (IDs)
  We use ID selectors to style our main, unique layout blocks.
*/
#container {
  width: 80%;
  max-width: 900px; /* Good for responsiveness */
...
/* STEP 3: Reusable Module Styles (Classes)
  We use CLASS selectors to style reusable "boxes" or "components".
*/
.section-box {
  margin-bottom: 25px;
  padding-bottom: 15px;
...
/* STEP 4: Grouping & Chaining
*/

/* GROUPING: Let's make all headings use a different font */
h1, h2 {
  font-family: 'Georgia', serif;
  color: #336699;
}

/* CHAINING: Let's target ONLY the h1 with the class .site-title */
/* (This is more specific than just 'h1' or '.site-title') */
h1.site-title {
  color: #ffffff; /* Overrides the blue color from the rule above */
  font-size: 40px;
  margin: 0;
}

/* CHAINING: Target the tagline <p> inside the header */
/* This is a "descendant" selector, which we'll cover, but notice how
...
/* STEP 5: Specificity Challenge (Add this to your code)
*/

/* Add a generic link style */
a {
  color: red;
}

/* QUESTION: Why are the social links (GitHub, LinkedIn)
  still blue/white, and not red?

  ANSWER (Write this in a comment):
  The selector '.social-link' (a CLASS) is MORE SPECIFIC
  than the selector 'a' (a TYPE). Therefore, the '.social-link'
  styles for 'background-color' and 'color' win the specificity war.
*/
```
